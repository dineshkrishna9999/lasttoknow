"""Tests for devpulse config management."""
...
def config_dir(tmp_path: Path) -> Path:
    ...
    return tmp_path / "devpulse"


@pytest.fixture
def config(config_dir: Path) -> DevPulseConfig:
    """Provide a DevPulseConfig with a temp directory."""
    return DevPulseConfig(config_dir=config_dir)


class TestConfigDirs:
    """Tests for config directory setup."""

    def test_creates_directory_on_load(self, config: DevPulseConfig, config_dir: Path) -> None:
        config.load()
        assert config_dir.exists()

    def test_config_file_paths(self, config: DevPulseConfig, config_dir: Path) -> None:
...
class TestSettings:
    ...
    def test_default_model_is_none(self, config: DevPulseConfig) -> None:
        assert config.model is None

    def test_set_model(self, config: DevPulseConfig) -> None:
        config.model = "azure/gpt-4.1"
        assert config.model == "azure/gpt-4.1"

    def test_default_sources(self, config: DevPulseConfig) -> None:
        assert "pypi" in config.sources
        assert "github" in config.sources
        assert "hackernews" in config.sources

    def test_default_days(self, config: DevPulseConfig) -> None:
        assert config.default_days == 7

    def test_save_and_reload_settings(self, config_dir: Path) -> None:
        # Save settings
        config1 = DevPulseConfig(config_dir=config_dir)
        config1.model = "gpt-4o"
        config1.save_settings()

        # Reload in a new instance
        config2 = DevPulseConfig(config_dir=config_dir)
        config2.load()
    ...
    def test_corrupted_config_uses_defaults(self, config_dir: Path) -> None:
        ...
        config = DevPulseConfig(config_dir=config_dir)
        config.load()
...
class TestTrackedItems:
    ...
    def test_empty_by_default(self, config: DevPulseConfig) -> None:
        assert config.tracked_items == []

    def test_add_pypi_item(self, config: DevPulseConfig) -> None:
    ...
    def test_add_github_item(self, config: DevPulseConfig) -> None:
    ...
    def test_remove_nonexistent_returns_false(self, config: DevPulseConfig) -> None:
        assert config.remove_item("doesnt-exist") is False

    def test_get_item(self, config: DevPulseConfig) -> None:
        config.add_item("litellm", ItemType.PYPI)
        item = config.get_item("litellm")
        assert item is not None
        assert item.name == "litellm"

    def test_get_item_not_found(self, config: DevPulseConfig) -> None:
        assert config.get_item("nope") is None

    def test_update_last_checked(self, config: DevPulseConfig) -> None:
        config.add_item("litellm", ItemType.PYPI)
    ...
    def test_clear_all(self, config: DevPulseConfig) -> None:
        config.add_item("litellm", ItemType.PYPI)
        config.add_item("click", ItemType.PYPI)
    ...
    def test_tracked_items_returns_copy(self, config: DevPulseConfig) -> None:
...
class TestPersistence:
    ...
    def test_tracked_file_is_valid_json(self, config: DevPulseConfig) -> None:
        config.add_item("litellm", ItemType.PYPI)
